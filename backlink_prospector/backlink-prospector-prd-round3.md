...
**Version:** 3.0
**Created:** 2025-01-19
**Updated:** 2025-01-19 (Round 3 - Final)
**Author:** Product Team
...
- **Scalable**: Process thousands of domains concurrently without manual intervention
...
**FR-3.2:** System must checkpoint progress after each batch completion. Checkpoints must persist to external storage (not ephemeral container storage).

**FR-3.3:** If an individual domain fails after 3 retry attempts, system must:
- Log the failure with detailed error message
- Mark domain as "failed" in database
- Continue processing remaining domains (do not abort job)

**FR-3.4:** System must implement exponential backoff for transient errors: 1s, 2s, 4s between retries

**FR-3.5:** System must detect and handle external service failures gracefully:
- If ScrapingBee API is unreachable for >5 minutes, pause job and alert user
- If database becomes unreachable, buffer results in memory (up to 1MB) and retry writes; if buffer full, pause job

**FR-3.6:** If more than 50% of first 100 domains fail, system must auto-pause and alert user: "High failure rate detected. Check domain list quality or network connectivity."

**FR-3.7:** System must implement idempotent result writes: If a worker processes the same URL twice (due to retry race condition), the system must upsert or skip the duplicate write without error

### FR-4: Scalability & Concurrency

**FR-4.1:** System must be capable of processing multiple domains concurrently to meet defined throughput targets (see NFR-1.3)

**FR-4.2:** System must enforce per-domain rate limiting: max 1 request per second per unique domain (FQDN), regardless of how many workers are active
...
- Buffer results in memory (up to configurable limit, default 100 results per worker)
- Slow down crawling rate (sleep between requests) to match DB write capacity
...
**NFR-1.3:** Target throughput: 1,000+ domains per hour at scale (measured across all concurrent jobs)

**NFR-1.4:** System must support horizontal scaling: Architecture must allow adding more workers to increase throughput (not limited to single-machine performance)
...
**NFR-3.3:** Target job completion time: Process 5,000 domains in under 8 hours (avg 625 domains/hour)
...
**PM-5: Global Concurrency Compliance**
...
| **Global Concurrency Limit Breach** | Critical - ScrapingBee account banned | Medium | Implement strict global concurrency cap enforced across all jobs. Log concurrency metrics every minute. Alert if cap exceeded. Dynamically throttle new job starts if cap is reached. |
...
| **External Service Downtime** | Medium - ScrapingBee/Cloud SQL unavailable | Low | Graceful degradation: If ScrapingBee down for >5 minutes, pause job and alert user (don't silently fail all domains). Retry logic with exponential backoff for transient errors. For DB downtime: buffer results in memory (up to 1MB per worker) and retry writes. |
| **Database Connection Exhaustion** | Critical - All jobs fail | Medium | Implement connection pooling (e.g., PgBouncer). Enforce hard limit on max concurrent workers (must be < DB max connections). Monitor active connections; alert at 80% of max. |
| **Export Memory Overflow** | High - Users can't download results | Medium | Mandate streaming writes for export generation (row-by-row from DB to cloud storage). Never load full dataset into memory. Test with 50,000-domain exports to ensure <1GB memory usage. |
...
**D-6:** Distributed cache/lock store (Redis or Memcached) for:
- Global concurrency limiting across workers
- Per-domain rate limiting coordination
...
| **Worker** | A compute instance (container, VM, serverless function) that processes domains concurrently. |
...
| **Soft Delete** | Marking data as deleted (invisible to users) but retaining in database for recovery, vs. hard delete (permanent removal). |
